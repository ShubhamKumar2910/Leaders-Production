({
    doInit : function(component, event, helper) { 
        helper.fetchAssignmentHelper(component, event,'',helper);
  ...
  /*  recordOperation:function(component, event, helper){
    ...
    navigate:function(component,event,helper){
        ...
        if(selectedRecord.length==10 || selectedRecord.length>3)
        {
                ...
                toastEvent.setParams({
                        "title": "Error!",
						"message": "You can't select more than 3 records",
                        "type":"Error"
                    });
                    toastEvent.fire();
        ...
        if(selectedRecord.length>1 && selectedRecord.length<4 ){
          ...
          var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        "title": "Error!",
                        "message": "Please select aleast more than 1 record",
                        "type":"Error"
                    });
                    toastEvent.fire();
        }    
    ...
    fetchAssignment : function(component, event, helper) {
        component.set("v.showSpinner",true);
        var params = event.getParam('arguments');
        if (params) {
            var searchText = params.searchString;
            
            helper.fetchAssignmentHelper(component, event,searchText,helper);
