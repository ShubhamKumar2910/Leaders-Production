({
    ...
    viewBioHelper : function(component, event, helper) {
        debugger;
        component.set("v.checkSpinner",true);
        var action = component.get("c.getBioAttId");
        action.setParams({
            "conId": component.get("v.recordId")
        });
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var res = response.getReturnValue();
                component.set('v.selectedBioId',res);
                var isbio = component.get("v.isBioPresent");
                if(isbio == true){
    ...
    saveBiographyList:function(component,event,helper){
        debugger;
        var assLang = component.get("v.mandateRecId"); 
        component.set("v.checkSpinner",true);
        ...
        var brdResult=false,proResult=false,eduResult=false,expResult=false;
        
        /*if(brdResult != undefined){
             brdResult = brdCmp.validateMe();    
        }
        if(proResult != undefined){
             proResult = proCmp.validateMe();    
        }
        if(eduResult != undefined){
             eduResult = eduCmp.validateMe();   
        }
        if(expResult != undefined){
             expResult = expCmp.validateMe();    
        }*/
        ...
        if(!brdResult && !proResult && !eduResult && !expResult){
            ...
            if(expList.length >0){
                ...
                for(var i=0;i<expList.length;i++){
                    if( expList[i].record.Current_Title__c == true){
                        count++;
                    }
                }
                if(count > 1){
                    swal.fire({
                        title: "Error!",
                        text: "you can not select multiple Current title checkbox.",
                        type: "error",
                        timer: 3000
                    });
                    component.set("v.checkSpinner",false);
